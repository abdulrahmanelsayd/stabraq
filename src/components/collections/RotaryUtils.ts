...
export class BentPlaneGeometry extends THREE.PlaneGeometry {
    constructor(radius: number, ...args: [width?: number, height?: number, widthSegments?: number, heightSegments?: number]) {
        super(...args)
        ...
        for (let i = 0; i < uv.count; i++) {
            let uvRatio = uv.getX(i) // Fixed mirror issue (was 1 - uv.getX(i))
            let y = pos.getY(i)
